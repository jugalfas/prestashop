...
class QuoteRequestFrontQuoteRequestModuleFrontController extends ModuleFrontController
{
    ...
    public function setMedia()
    {
        ...
        $this->addCSS(Tools::getHttpHost(true) . __PS_BASE_URI__ . 'modules/quoterequest/views/css/custom-styles.css');
    }
    ...
    public function initContent()
    {
        ...
        if (Tools::getValue('action')) {
            if (Tools::getValue('action') === 'get_product_details') {
            ...
            } else if (Tools::getValue('action') === 'load_configuration') {
                $id_product = (int) Tools::getValue('id_product');
                $hookContent = Hook::exec('displayQuoteRequest', array(
                    'id_product' => $id_product
                ));

                $response = ['status' => true, 'content' => $hookContent];
                die(json_encode($response));
            } else if (Tools::getValue('action') === 'send_quote') {
                $customer_email = Tools::getValue('customer_email');
                $quotes = Tools::getValue('quotes');
                $quote_title = Tools::getValue('quote_title');
                ...
                $quote = new KDQuote();
                $quote->id_customer = $customer->id ?? 0;
                $quote->id_order = 0;
                $quote->id_lang = $language->id;
                $quote->id_cart = 0;
                $quote->id_currency = $currency->id;
                $quote->title = $quote_title;
                $quote->email = $customer_email;
                $quote->allow_partial = 0;
                $quote->current_status = 0;
                $quote->note = '-';
                $quote->date_add = date('Y-m-d H:i:s'); // Current date and time
                $quote->date_upd = date('Y-m-d H:i:s'); // Current date and time
                ...
                try {
                    ...
                    $target_dir = $module_path . "uploads/";

                    if (!is_dir($target_dir)) {
                        mkdir($target_dir, 0777, true); // Create the directory with read/write/execute permissions
                    }
                    
                    foreach ($quotes as $quote_key => &$quote_data) {
                        $file_names = []; // Initialize an array to store the file names for each quote

                        $response = [];
                        foreach ($_FILES['quote_images']['tmp_name'][$quote_key] as $key => $tmp_name) {
                            if ($_FILES['quote_images']['error'][$quote_key][$key] === 0) {
                                $file_name = $_FILES['quote_images']['name'][$quote_key][$key];
                                $target_file = $target_dir . basename($file_name);
                                
                                if (move_uploaded_file($tmp_name, $target_file)) {
                                    $file_names[] = $file_name; // Add the file name to the array
                                }
                            }
                        }

                        $quote_data['configuration'][] = ['additional_information' => $quote_data['additional_information']];
                        $quote_product = new KDQuoteProduct();
                        $quote_product->id_quote = $quote->id;
                        $quote_product->id_product = $quote_data['product_id'];
                        $quote_product->id_product_attribute = 0;
                        $quote_product->status = 0;
                        $quote_product->details = json_encode($quote_data['configuration']);
                    ...
                    $template = 'quote_request_admin_notification';
                    ...
                    $email = $this->context->customer->isLogged() ? '' : 'email=' . urlencode($customer_email);

                    // Generate the module link
                    $link = $this->context->link->getModuleLink('quoterequest', 'frontquotelist');

                    // Append the email query parameter if the customer is not logged in
                    if (!$this->context->customer->isLogged()) {
                        $link .= (strpos($link, '?') === false ? '?' : '&') . $email;
                    }

                    $response = ['status' => true, 'message' => 'Quote Request Sent Successfully', 'redirect_link' => $link];
                } catch (\Throwable $th) {
                    $response = ['status' => false, 'message' => 'Quote Request Not Sent'];
                }
                die(json_encode($response));
            }
        }

        // Retrieve all active products using ObjectModel method
        $productsByCategory = [];
        ...
        // Assign the active products to the template
        $this->context->smarty->assign('ajax_link', $this->context->link->getModuleLink('quoterequest', 'frontquoterequest'));
        $this->context->smarty->assign('products', $productsByCategory);
        $this->context->smarty->assign('customer_email', $customer_email);


        if (version_compare(_PS_VERSION_, '1.7.0', '>=') === true) {
            $this->setTemplate('module:quoterequest/views/templates/front/quote_request.tpl');
        } else {
            $this->setTemplate('quote_request.tpl');
        }
    }
