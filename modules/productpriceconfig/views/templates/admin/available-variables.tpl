...
<script type="text/javascript">
	...
	function markBlockedFilters() {
		var $templates = $('.af_template');

		$templates.each(function(){
			$('.filter-group-item[data-key="'+$(this).data('id')+'"]').addClass('blocked');
		});
...
{/literal}
	...
	{$quick_search = $available_variables|count > 200}
	<div class="filter-group">
		<div class="filter-group-title">
			<h4 class="inline-block">{$subtitle|escape:'html':'UTF-8'}</h4>
			<label class="inline-block">
				<input type="checkbox" class="check-all">
				{l s='select available' mod='productpriceconfig'}
				<span class="total">({$available_variables|count})</span>
			</label>
			{if $quick_search}
				<div class="pull-right">
					<div class="inline-block">
						<input type="text" class="quick-search" placeholder="{l s='Quick search' mod='productpriceconfig'}">
					</div>
					<div class="inline-block hidden">
						<select name="" class="sort-by">
							<option value="position">{l s='Sort by position' mod='productpriceconfig'}</option>
							<option value="name">{l s='Sort by name' mod='productpriceconfig'}</option>
						</select>
					</div>
				</div>
			{/if}
		...
		<div class="filter-items-list">
			{foreach $available_variables as $key => $variable}<a href="#" data-key="{$key|escape:'html':'UTF-8'}" class="filter-group-item">
				<span class="item-id{if !$variable.id} hidden{/if}">{$variable.id|intval}</span>
				<span class="item-name">{$variable.name|escape:'html':'UTF-8'}</span>
				<span class="item-position hidden">{$variable.position|intval}</span>
			</a>{/foreach}
			{if $quick_search}
				<div class="alert-warning no-results hidden">{l s='No matches' mod='productpriceconfig'}</div>
			{/if}
		</div>
	</div>
<div class="available-filters-footer text-center">
	<button class="btn btn-primary btn-blocked addSelectedFilters">
		{l s='Add selected' mod='productpriceconfig'} <span class="total-selected">(0)</span>
	</button>
</div>
{* since 2.8.0 *}
