...
class HTMLTemplateInvoiceCore extends HTMLTemplate
{
    ...
    public function __construct(OrderInvoice $order_invoice, Smarty $smarty, $bulk_mode = false)
    {
        ...
        $this->order_cart_rules = $this->order->getCartRules($this->order_invoice->id);
        $this->smarty = $smarty;
...
public function getHeader()
{
    ...
    $path_logo = '/img/logo.png'; // öffentlicher Pfad;
    $shop_address = $this->getShopAddress();
    $shop_phone = Configuration::get('PS_SHOP_PHONE'); // <- NEU hinzugefügt
    $shop_email = Configuration::get('PS_SHOP_EMAIL');

    $this->smarty->assign([
        'header'       => Context::getContext()->getTranslator()->trans('Invoice', [], 'Shop.Pdf'),
        'shop_logo'    => $path_logo,
        'shop_address' => $shop_address,
        'shop_phone'   => $shop_phone, // <- jetzt definiert
    ...
    public function getContent()
    {
       ...
       // Vorher definieren wir die Variablen sicher:
        $orderReference = '';
        $orderDate = '';
        $invoiceDate = '';

        if ($this->order) {
            $orderReference = $this->order->getUniqReference();
            $orderDate = $this->order->date_add;
        }

        if ($this->order_invoice) {
            $invoiceDate = $this->order_invoice->date_add;
        }
$invoice_address = new Address($this->order->id_address_invoice);
$vat_number = $invoice_address->vat_number;

        // Dann weisen wir alle Variablen zu:
        $this->smarty->assign(array(
            ...
            'order_invoice'    => $this->order_invoice,
            'order_details'    => $this->order_invoice->getProductsDetail(),
            'cart_rules'       => $this->order_cart_rules,
            'delivery_address' => $formatted_delivery_address,
            'invoice_address'  => $formatted_invoice_address,
            'vat_number' => $vat_number,
            
            // Neue Zuweisung für das Tab
            'addresses' => [
                'invoice' => $formatted_invoice_address,
                'delivery' => $formatted_delivery_address
            ],
        ));

        $this->smarty->assign('layout', [
            'product' => ['width' => 60],
            'tax_code' => ['width' => 10],
            'reference' => ['width' => 10],
            'unit_price_tax_excl' => ['width' => 10],
            'quantity' => ['width' => 5],
            'total_tax_excl' => ['width' => 5],
            '_colCount' => 6,
            'display_product_images' => true, // ✅ Diese Zeile ist entscheidend!
        ]);

        // 👉 Hier das Footer-Array zusätzlich zuweisen!
        $this->smarty->assign('footer', $footer);

        $this->smarty->assign('invoice_number', $this->order_invoice->getInvoiceNumberFormatted($this->order->id_lang));
        $this->smarty->assign('payment', $this->order->payment);

        $this->smarty->assign('legal_free_text', $legal_free_text);

        // Diese Zeile ist entscheidend:
        $this->smarty->assign('display_product_images', true);
        ...
        $tpls = [
            ...
            'footer' => $this->smarty->fetch($this->getTemplate('invoice.footer')),
