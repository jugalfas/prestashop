...
/* FAQs - Acoordion */
$('.accordion-op--question').click(function(){
    if($(this).parent('.accordion-op--question-wrapper').hasClass('active')){
         $('.accordion-op--question-wrapper').removeClass('active');
    }else{
        $('.accordion-op--question-wrapper').removeClass('active');
        $(this).parent('.accordion-op--question-wrapper').addClass('active');
    }
})



/* alphabetical ordered megamenu */

//Taking Full <li>
var item = $('.pos-menu-row.row.alphabetical-ordered > div > ul').find('li a');
var currentLetter = "";
...
item.each(function(e, item){
  ...
  if(currentLetter !== firstLetter){
    $('<div class="alphabetical-header"><p class="column_title">' + firstLetter + '</p></div>').insertBefore($(item).parent('li'));    
  }
...
/* DISABLE CUSTOM FIELDS AT ORDER DETAIL PAGE*/
if ($("body#order-detail").length > 0) {
	$('textarea.form-control').attr('disabled', true);
	$('select.select.form-control').attr('disabled', true);
}