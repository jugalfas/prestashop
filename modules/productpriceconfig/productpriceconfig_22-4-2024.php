...
class ProductPriceConfig extends Module
{
    ...
    public function install()
    {
        if (
            ...
            $sql11 = "CREATE TABLE `ps_alert_messages` (
    ...
    public function hookDisplayHeader()
    {
        ...
        if (Tools::getValue('controller') == 'product') {
            ...
            if (count($rules_list)) {
                ...
                $js .= '$("ul[data-variable-name] li, input[data-variable-name]").each(function() {'
                ...
                foreach ($rules_list as &$rules) {
                    ...
                    if (!empty($rule_data)) {
                        ...
                        foreach ($rule_data as $rule) {
                            ...
                            if ($variable_name) {
                                if ($varObj->type == 2) {
                                    $variable_text = '$("ul[data-variable_id=\'' . $rule['variable'] . '\'] li.selected:first").text()';
                                } else {
                            ...
                            if (is_array($disallow)) {
                                ...
                                foreach ($disallow_variables as $variable) {
                                    ...
                                    $rule_text .= '$("input:hidden.btn-select-input[data-variable_id=\'' . $variable . '\']").val("");'
                                        ...
                                        . '$("ul[data-variable_id=\'' . $variable . '\'] li").each(function() {';
                                    foreach ($disallow_options as $option) {
                                        $optionObj = new KDOption($option);
                                        $name = $optionObj->label[$current_language_id];
                                        $disallowed  .= $name . ', ';
                                        ...
                                        $rule_text .= 'if ($(this).text() === "' . $name . '") {
                                            ...
                                            . '$(".btn-select-value[data-variable_id=\'' . $variable . '\']").text("' . $this->l('Bitte auswählen') . '");'  // Set the default label
                                            . '}
                                    ...
                                    $rule_text .= '});'
                                        . 'var ul = $("ul[data-variable_id=\'' . $variable . '\']");'
            ...
            $alert_messages_combination = 'function alert_messages_combination(){';
            if (count($alert_message_list) > 0) {
                ...
                foreach ($alert_message_list as $key => $alert_message) {
                    ...
                    $message_text .= 'if (';
                    if ($variable->type == 2) {
                        $second_variable_condition_text = '$("ul[data-variable-name=\'' . $variable->name . '\'] li.selected").text()';
                    } else {
    ...
    public function ajaxGetFilteredVariables($params)
    {
        ...
        $price_data = array();
        $formula_width = $formula_height = 0;
        $priceFormatter = new PriceFormatter();
        ...
        foreach ($available_product_variables as $data) {
            ...
            if (isset($params['variable_' . $data['id_product_variable']])) {
                ...
                if ($varObj->type == 1) {
                    ...
                    $value_weight = $value_price = $custom_input;
    ...
    public function ajaxCalcuateTotals()
    {
    }
    ...
    public function ajaxAddToCart($params)
    {
        ...
        if ($width_price_factor || $height_price_factor) {
            $final_price_factor = max(array($width_price_factor, $height_price_factor));
            if (strpos($formula_price, $autosize_formula_string) !== false) {
                $formula_price = str_replace($autosize_formula_string, $final_price_factor, $formula_price);
            }
        }
        ...
        // $total_weight = 1.2;
        $total_weight = $m->evaluate($formula_weight);
