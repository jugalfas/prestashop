...
<script type="text/javascript">
    ...
    var must_select_layout = "{l s='You must select the layout' mod='productpriceconfig'}";
    var image_only = "{l s='You must select an image file only' mod='productpriceconfig'}";
    var max_size = "{l s='Please upload a smaller image, max size is 12 MB' mod='productpriceconfig'}";
    var reset_design = "{l s='You want to reset your design ?' mod='productpriceconfig'}";
    var back_string = "{l s='Back' mod='productpriceconfig'}";
    var ok_string = "{l s='OK' mod='productpriceconfig'}";
    var ok_str = "{l s='OK' mod='productpriceconfig'}";
    var crop_pic = "{l s='Crop this picture' mod='productpriceconfig'}";
    var cancel_str = "{l s='Cancel' mod='productpriceconfig'}";
    var delete_pic = "{l s='delete this picture' mod='productpriceconfig'}";
    var on_process = "{l s='Save in process' mod='productpriceconfig'}";
    var redirection = "{l s='redirection' mod='productpriceconfig'}";
    var success_save = "{l s='Your design was saved with success' mod='productpriceconfig'}";
    var error_save = "{l s='You must be connected for you can save your design' mod='productpriceconfig'}";
</script>


<script type="text/javascript" src="{$urls_site|escape:'htmlall':'UTF-8'}themes/core.js"></script>
<script type="text/javascript" src="{$urls_site|escape:'htmlall':'UTF-8'}modules/productpriceconfig/views/js/jquery-ui.min.js"></script>
<script type="text/javascript" src="{$urls_site|escape:'htmlall':'UTF-8'}modules/productpriceconfig/views/js/front.js"></script>
...
<div class="container">
	<div class="row" >
   <form id="register-form" action="{$urls_site|escape:'htmlall':'UTF-8'}index.php?fc=module&module=productpriceconfig&controller=frontproductpriceconfig" class="default-form" enctype="multipart/form-data" method="post">
	...
	<div class="step_list">
		...
		{foreach from=$product.customizations.fields item="field" }
				<div id="step_{$field.id_variable}" class="step_type_{$field.type} form-line step_group">
				 ...
				 {elseif $field.type == 'image'}
					 <div class="form-line-wrapper">
            			...
            			<a href="#" title="{l s='Import from your computer' mod='productpriceconfig'}" onchange="priview_img($('#fileupload'), '.preview');" class="cp-btn-more-pic fileinput-button">
									<span id="cp-img-lo-w">
											<i class="fa fa-refresh fa-spin" id="cp-img-lo"></i>
											<span class="cp-loader-number"></span>
									</span>
									<i class="fa fa-picture-o"></i>
									<samp>{l s='Browse' mod='productpriceconfig'}</samp>
					 ...
					 <div class="form-line-wrapper">
						...
						<ul id="gender-radio-btn-wrapper" class="step_options" >
							{foreach from=$field.options key=id_option item=option}
								<li id="step_option_{$field.id_variable}_{$option.id}" class="option_block option_group custom_{$field.type}" >
						...
						<div class="row">
						 <div class="col-xs-12">
							 <div class="step_options" id="collapse_{$field.id_variable}">
								 {foreach from=$field.options key=id_option item=option}
									 <div id="step_option_{$field.id_variable}_{$option.id}" class="option_block option_group custom_{$field.type} {if isset($field.selected) and in_array($option.id, $field.selected)} selected {/if}" >
...
<div class="col-xs-3">
	<div id="configurator_preview" class="" style="position: relative; top: 0px;">
		<div class="box tabbed_form">
			...
			<hr>
			<div class="customisation_item clearfix">
				<div class="custimg"><img  src="{$product.cover.bySize.large_default.url}" alt="{$product.cover.legend}" title="{$product.cover.legend}"  itemprop="image"></div>
				<div class="custDesc">
			...
			<br/><span style="font-size:10px; line-height:1em;">Please note we will send you a proof to approve before starting your order. You can change this at any time prior to the proof being approved.</span>
		</div>
	</div>
</div>
</div>
